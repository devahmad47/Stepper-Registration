...
const Contact = ({ onNext }) => {
  ...
  return (
    <>
      <div className="px-12 py-4 w-full max-w-lg mx-auto">
        <div className="font-bold text-xl w-full mb-2 justify-center  text-center">Password</div>
        <p className="text-gray-700 text-center text-xs text-custom-10">
          Enter your password for business profile
        </p>
  <form onSubmit={(e) => e.preventDefault()} className="w-full max-w-lg p-4 ">
    <div className="bg-white p-2 -mb-1">
      <div className="relative w-auto bg-inherit">
        <input
          type="password"
          id="password"
          required
    ...
    <div className="bg-white p-2 rounded-lg mt-2">
      ...
      <button
        onClick={validate}
        className="py-2 px-12 max-w-full flex justify-center items-center bg-[#07A6A9] text-white w-full transition ease-in duration-200 text-center text-sm font-light rounded-sm"
      >
        Continue
      </button>
    </div>
